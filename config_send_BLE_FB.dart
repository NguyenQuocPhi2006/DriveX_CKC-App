import 'package:flutter/material.dart';
import 'firebase.dart';
import 'more_settings.dart';
// import 'bluetooth_classic.dart';
import 'config_send_classic_BLE.dart';
import 'package:shared_preferences/shared_preferences.dart';

// Biến toàn cục isBluetoothMode quản lý chế độ: true = Bluetooth, false = Firebase
bool isBluetoothMode = true;

class ChoiceScreen extends StatefulWidget {
  const ChoiceScreen({super.key});

  @override
  State<ChoiceScreen> createState() => _ChoiceScreenState();
}

class _ChoiceScreenState extends State<ChoiceScreen> {
  @override
  void initState() {
    super.initState();
    _loadSavedMode(); // 2. Đọc lại lựa chọn cũ khi màn hình vừa mở lên
  }

  // Hàm đọc dữ liệu đã lưu trong bộ nhớ máy
  Future<void> _loadSavedMode() async {
    final prefs = await SharedPreferences.getInstance();
    setState(() {
      // Nếu chưa lưu lần nào thì mặc định là true (Bluetooth)
      isBluetoothMode = prefs.getBool('isBluetoothMode') ?? true;
    });
  }

  // Hàm lưu trạng thái vào bộ nhớ máy
  Future<void> _saveMode(bool value) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setBool('isBluetoothMode', value);
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.grey[150],
      appBar: AppBar(
        title: Text(
          'Chọn chế độ kết nối',
          style: TextStyle(fontFamily: selectedFont),
        ),
        centerTitle: true,
        elevation: 0,
      ),
      // Đã bọc trong SingleChildScrollView để chống tràn viền (overflow) khi xoay ngang
      body: SingleChildScrollView(
        child: Center(
          child: Padding(
            padding: const EdgeInsets.all(24.0),
            child: Column(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Text(
                  'Vui lòng chọn phương thức kết nối của you',
                  style: TextStyle(
                    fontSize: 16,
                    color: Colors.grey,
                    fontFamily: selectedFont,
                  ),
                ),
                const SizedBox(height: 20),

                // --- NÚT BLUETOOTH ---
                _buildChoiceButton(
                  title: 'Bluetooth',
                  subtitle: 'Kết nối thiết bị cục bộ qua sóng Bluetooth',
                  icon: Icons.bluetooth,
                  isSelected: isBluetoothMode == true,
                  onTap: () async {
                    await _saveMode(true);
                    setState(() {
                      isBluetoothMode = true;
                    });
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => const select_send_Classic_BLE(),
                      ),
                    );
                  },
                ),
                const SizedBox(height: 16),

                // --- NÚT FIREBASE ---
                _buildChoiceButton(
                  title: 'Firebase',
                  subtitle: 'Đồng bộ dữ liệu trực tuyến qua Cloud Database',
                  icon: Icons.cloud,
                  isSelected: isBluetoothMode == false,
                  onTap: () async {
                    await _saveMode(false);
                    setState(() {
                      isBluetoothMode = false;
                    });
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => const Option2Page(),
                      ),
                    );
                  },
                ),
              ],
            ),
          ),
        ),
      ),
    );
  }

  // Hàm phụ trợ tạo giao diện cho từng nút bấm có hiệu ứng đổi màu chuyên nghiệp
  Widget _buildChoiceButton({
    required String title,
    required String subtitle,
    required IconData icon,
    required bool isSelected,
    required VoidCallback onTap,
  }) {
    return AnimatedContainer(
      duration: const Duration(milliseconds: 300),
      decoration: BoxDecoration(
        color: isSelected ? Colors.blue.shade50 : Colors.white,
        borderRadius: BorderRadius.circular(16),
        border: Border.all(
          color: isSelected ? Colors.blue : Colors.grey.shade300,
          width: isSelected ? 2.5 : 1,
        ),
        boxShadow: [
          BoxShadow(
            color: Colors.grey.withOpacity(0.1),
            spreadRadius: 2,
            blurRadius: 8,
            offset: const Offset(0, 4),
          ),
        ],
      ),
      child: Material(
        color: Colors.transparent,
        child: InkWell(
          borderRadius: BorderRadius.circular(16),
          onTap: onTap,
          child: Padding(
            padding: const EdgeInsets.all(20.0),
            child: Row(
              children: [
                Container(
                  padding: const EdgeInsets.all(12),
                  decoration: BoxDecoration(
                    color: isSelected ? Colors.blue : Colors.grey.shade200,
                    shape: BoxShape.circle,
                  ),
                  child: Icon(
                    icon,
                    color: isSelected ? Colors.white : Colors.grey.shade700,
                    size: 28,
                  ),
                ),
                const SizedBox(width: 16),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(
                        title,
                        style: TextStyle(
                          fontSize: 18,
                          fontWeight: FontWeight.bold,
                          fontFamily: selectedFont,
                          color: isSelected
                              ? Colors.blue.shade900
                              : Colors.black87,
                        ),
                      ),
                      const SizedBox(height: 4),
                      Text(
                        subtitle,
                        style: TextStyle(
                          fontSize: 13,
                          color: Colors.grey.shade600,
                          fontFamily: selectedFont,
                        ),
                      ),
                    ],
                  ),
                ),
                if (isSelected)
                  const Icon(Icons.check_circle, color: Colors.blue, size: 24),
              ],
            ),
          ),
        ),
      ),
    );
  }
}